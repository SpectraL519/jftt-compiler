...
* Build the virtual machine: `make vm`
    ...
    > **NOTE:** The virtual machine expects a file with the **machine readable** code (by convention files with `.mr` convention) as an input.

    > **NOTE:** The `cln-run-vm` executable is built with the `libcln-dev 1.3.6` library.
