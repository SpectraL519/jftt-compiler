...
[[nodiscard]] std::string load(const jftt::architecture::vm_register_discriminator reg) noexcept {
    static const std::string instruction{"LOAD "};
    return instruction + jftt::architecture::as_string(reg);
}

[[nodiscard]] std::string store(const jftt::architecture::vm_register_discriminator reg) noexcept {
